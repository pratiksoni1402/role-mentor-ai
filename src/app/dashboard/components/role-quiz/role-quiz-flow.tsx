...
export default function RoleQuizFlow() {
  ...
  const handleGenerateQuiz = async () => {
    setLoading(true)
    try {
      const res = await axios.post("/dashboard/components/roles/api/quiz-generation", { role: finalRole })
  ...
  const handleEvaluate = async () => {
    setLoading(true)
    try {
      const res = await axios.post("/dashboard/components/roles/api/quiz-evaluation", {
        role: finalRole,
        questions,
        answers,
      })
  ...
  const handleGenerateTutorial = async () => {
    ...
    setLoading(true)
    try {
      const res = await axios.post("/dashboard/components/roles/api/tutorial-generation", {
        role: finalRole,
        evaluation,
      })
      ...
      let parsed: Tutorial
      try {
        parsed = typeof res.data.tutorial === "string" ? JSON.parse(res.data.tutorial) : res.data.tutorial
      } catch (err) {
  ...
  return (
    <section className="mx-auto max-w-4xl p-6 md:p-8 rounded-2xl border bg-card shadow-sm">
      {!tutorial && (
        <header className="mb-6 md:mb-8">
          <h1 className="text-balance text-2xl md:text-3xl font-semibold tracking-tight">Role-based Skills Quiz</h1>
          <p className="text-muted-foreground mt-1 text-pretty">
            Select your role, answer quick questions, then review tailored guidance.
          </p>
      ...
      {step === 3 && (
        <QuizQuestions
          questions={questions}
          answers={answers}
          setAnswer={(index, value) => setAnswers({ ...answers, [index]: value })}
